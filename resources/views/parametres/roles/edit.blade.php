...
@section('page-title', 'Modifier le rôle « ' . $role->name . ' »')
@section('page-title-info')
    <nav class="flex items-center gap-1.5 text-sm flex-wrap" aria-label="Fil d'Ariane">
        ...
        <span class="text-slate-300 dark:text-slate-600">/</span>
        <span class="text-slate-700 dark:text-slate-200 font-semibold">{{ ucfirst($role->name) }}</span>
    </nav>
@endsection
...
@php
    ...
    $groups = [
        ...
        'Menus GED' => array_filter($permissions->pluck('name')->toArray(), fn ($p) => in_array($p, ['dashboard.view', 'organigramme.view'], true) || str_starts_with($p, 'parametres.')),
...
<form id="form-role-edit" action="{{ route('parametres.roles.update', $role) }}" method="POST" class="space-y-6">
    @csrf
    @method('PUT')
    <div class="bg-white dark:bg-slate-800 rounded-xl shadow-md border border-slate-100 dark:border-slate-700 overflow-hidden">
        <div class="px-6 py-4 border-b border-slate-200 dark:border-slate-600 bg-slate-50/80 dark:bg-slate-700/30 space-y-4">
            <div class="max-w-md">
                <label class="block text-sm font-semibold text-slate-700 dark:text-slate-300 mb-2">Nom du rôle *</label>
                <input type="text" name="name" value="{{ old('name', $role->name) }}" required maxlength="255" class="w-full px-4 py-2 rounded-lg border border-slate-300 dark:border-slate-600 dark:bg-slate-700 dark:text-white @error('name') border-red-500 @enderror">
                @error('name')<p class="mt-1 text-sm text-red-500">{{ $message }}</p>@enderror
            </div>
            <p class="text-sm text-slate-600 dark:text-slate-400">
                Cochez les permissions que les utilisateurs ayant ce rôle pourront exercer.
            </p>
        </div>
                ...
                <div>
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-3" data-group="{{ Str::slug($title) }}">
                        ...
                        <label class="flex items-start gap-3 cursor-pointer p-3 rounded-xl border border-slate-200 dark:border-slate-600 hover:bg-slate-50 dark:hover:bg-slate-700/30 transition-colors">
                            <input type="checkbox" name="permissions[]" value="{{ $p }}" {{ in_array($p, old('permissions', $rolePerms)) ? 'checked' : '' }} class="mt-0.5 rounded border-slate-300 text-emerald-600 focus:ring-emerald-500 w-4 h-4 flex-shrink-0">
                            <div class="min-w-0">
    ...
    <div class="mt-8 flex gap-4">
        <button type="submit" id="btn-role-edit-submit" class="px-6 py-2.5 rounded-lg bg-[#00b464] text-white font-semibold hover:bg-[#00a055] btn-submit-loading">Mettre à jour</button>
    ...
    <aside class="flex-1 min-w-0 sticky top-24 bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-700 p-6">
        ...
        <ul class="space-y-3 text-sm text-slate-600 dark:text-slate-400">
            <li><strong class="text-slate-800 dark:text-slate-200">Modification</strong> — Modifiez le nom du rôle ou les permissions associées. Les utilisateurs ayant ce rôle héritent automatiquement des changements.</li>
        </ul>
    </aside>
</div>
@push('styles')
...
#btn-role-edit-submit .btn-submit-spinner,
#btn-role-edit-submit .auth-spinner {
    ...
    animation: role-edit-spin 0.6s linear infinite !important;
    vertical-align: -0.2em !important;
    margin-right: 0.35rem !important;
    flex-shrink: 0 !important;
}
@keyframes role-edit-spin {
...
<script>
(function(){var f=document.getElementById('form-role-edit'),b=document.getElementById('btn-role-edit-submit');if(!f||!b)return;f.addEventListener('submit',function(){if(b.dataset.loading==='1')return;b.dataset.loading='1';b.innerHTML='<span class="btn-submit-spinner"></span> Mise à jour...';b.disabled=true;});})();
</script>
@endpush
@endsection
