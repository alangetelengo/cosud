...
@section('page-title', 'Modifier l\'étape de workflow')
@section('page-title-info')
    <nav class="flex items-center gap-1.5 text-sm flex-wrap" aria-label="Fil d'Ariane">
        ...
        <span class="text-slate-300">/</span>
        <span class="text-slate-700 dark:text-slate-200 font-semibold">{{ $etape->nom }}</span>
    </nav>
@endsection
...
<div class="flex flex-row gap-8 w-full items-stretch">
    ...
    <div class="flex-1 min-w-0 bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-700 p-8">
        <form action="{{ route('parametres.workflow.update', $etape) }}" method="POST" class="space-y-6">
            @csrf
            @method('PUT')
            @include('parametres.workflow._fields', ['etape' => $etape, 'types' => $types, 'fonctions' => $fonctions, 'services' => $services, 'utilisateurs' => $utilisateurs, 'etapes' => $etapes])
