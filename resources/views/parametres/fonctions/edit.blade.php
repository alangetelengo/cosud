...
@section('page-title', 'Modifier la fonction métier')
@section('page-title-info')
    <nav class="flex items-center gap-1.5 text-sm flex-wrap" aria-label="Fil d'Ariane">
        ...
        <span class="text-slate-300 dark:text-slate-600">/</span>
        <span class="text-slate-700 dark:text-slate-200 font-semibold">{{ $fonction->libelle }}</span>
    </nav>
@endsection
...
<div class="flex flex-row gap-8 w-full items-stretch">
    <div class="flex-1 min-w-0 bg-white dark:bg-slate-800 rounded-xl shadow-md border border-slate-100 dark:border-slate-700 p-8">
        <form id="form-fonction-edit" action="{{ route('parametres.fonctions.update', $fonction) }}" method="POST">
            @csrf
            @method('PUT')
            <div class="space-y-6">
                <div>
                    <label class="block text-sm font-semibold text-slate-700 dark:text-slate-300 mb-2">Code *</label>
                    <input type="text" name="code" value="{{ old('code', $fonction->code) }}" required pattern="[a-z0-9_]+" maxlength="50"
                ...
                <div>
                    ...
                    <input type="text" name="libelle" value="{{ old('libelle', $fonction->libelle) }}" required maxlength="255"
                ...
                <div>
                    ...
                    <textarea name="description" rows="3" class="w-full px-4 py-2 rounded-lg border border-slate-300 dark:border-slate-600 dark:bg-slate-700 dark:text-white">{{ old('description', $fonction->description) }}</textarea>
                </div>
                <div>
                    <label class="flex items-center gap-2 cursor-pointer">
                        <input type="hidden" name="actif" value="0">
                        <input type="checkbox" name="actif" value="1" {{ old('actif', $fonction->actif) ? 'checked' : '' }} class="rounded border-slate-300 text-emerald-600 focus:ring-emerald-500 w-4 h-4">
                        <span class="text-sm font-semibold text-slate-700 dark:text-slate-300">Actif</span>
                    </label>
                </div>
            </div>
            <div class="mt-8 flex gap-4">
                <button type="submit" id="btn-fonction-edit-submit" class="px-6 py-2.5 rounded-lg bg-[#00b464] text-white font-semibold hover:bg-[#00a055] btn-submit-loading">Mettre à jour</button>
    ...
    <aside class="flex-1 min-w-0 sticky top-24 bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-700 p-6">
        ...
        <ul class="space-y-3 text-sm text-slate-600 dark:text-slate-400">
            <li><strong class="text-slate-800 dark:text-slate-200">Modification</strong> — Le code et le libellé sont visibles dans l’organigramme et les écrans d’affectation. Désactiver une fonction la retire des listes de choix pour les nouvelles affectations si votre logique métier le prévoit.</li>
        </ul>
    </aside>
</div>
@push('styles')
<style>
#btn-fonction-edit-submit .btn-submit-spinner,
#btn-fonction-edit-submit .auth-spinner {
    ...
    animation: fonction-edit-spin 0.6s linear infinite !important;
    vertical-align: -0.2em !important;
    margin-right: 0.35rem !important;
    flex-shrink: 0 !important;
}
@keyframes fonction-edit-spin {
...
@push('scripts')
<script>
(function(){var f=document.getElementById('form-fonction-edit'),b=document.getElementById('btn-fonction-edit-submit');if(!f||!b)return;f.addEventListener('submit',function(){if(b.dataset.loading==='1')return;b.dataset.loading='1';b.innerHTML='<span class="btn-submit-spinner"></span> Mise à jour...';b.disabled=true;});})();
</script>
@endpush
@endsection
