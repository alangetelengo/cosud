...
@section('page-title', 'Fonctions métier')
@section('page-title-info')
    <nav class="flex items-center gap-1.5 text-sm flex-wrap" aria-label="Fil d'Ariane">
        ...
        <span class="text-slate-700 dark:text-slate-200 font-semibold">Fonctions métier</span>
    </nav>
@endsection

@section('btn-create')
    <a href="{{ route('parametres.fonctions.create') }}" class="inline-flex items-center gap-2 px-5 py-2.5 rounded-lg bg-[#00b464] text-white font-semibold hover:bg-[#00a055] shadow-sm hover:shadow transition-all no-underline">
...
<div class="space-y-6">
    ...
    <p class="text-slate-600 dark:text-slate-400 text-sm max-w-3xl leading-relaxed">
        Les fonctions métier définissent le rôle attendu pour la validation hiérarchique sur chaque structure. Les titulaires sont les utilisateurs affectés à la structure avec la même fonction (voir <a href="{{ route('parametres.structures.index') }}" class="text-emerald-600 dark:text-emerald-400 font-semibold hover:underline">Organigramme</a> et affectations utilisateur).
    </p>
            ...
            <table class="w-full">
                ...
                <tbody class="divide-y divide-slate-100 dark:divide-slate-700/80">
                    ...
                    <tr class="group hover:bg-emerald-50/50 dark:hover:bg-slate-700/40 transition-colors duration-200">
                        <td class="px-6 py-4">
                            <span class="font-mono text-sm font-medium text-slate-800 dark:text-slate-200 bg-slate-100 dark:bg-slate-700 px-2.5 py-1 rounded-lg">{{ $f->code }}</span>
                        </td>
                        <td class="px-6 py-4 font-medium text-slate-800 dark:text-slate-200">{{ $f->libelle }}</td>
                        <td class="px-6 py-4">
                            <span class="inline-flex items-center px-3 py-1.5 rounded-full text-xs font-semibold {{ $f->actif ? 'bg-emerald-100 dark:bg-emerald-900/50 text-emerald-700 dark:text-emerald-300 border border-emerald-200/50 dark:border-emerald-700/50' : 'bg-slate-200 dark:bg-slate-600 text-slate-600 dark:text-slate-300 border border-slate-300/50 dark:border-slate-500/30' }}">
                                {{ $f->actif ? 'Actif' : 'Inactif' }}
                            </span>
                        </td>
                        <td class="px-6 py-4">
                            <div class="flex items-center justify-end gap-1">
                                <a href="{{ route('parametres.fonctions.edit', $f) }}"
                                ...
                                <form action="{{ route('parametres.fonctions.destroy', $f) }}" method="POST" class="inline">
                                    @csrf
                                    @method('DELETE')
                                    <button type="button" onclick="flashAlert('Supprimer cette fonction métier ?', this.closest('form'), {icon:'🗑️', danger:true, confirmText:'Supprimer'})" class="p-2.5 rounded-xl text-slate-400 hover:text-red-600 hover:bg-red-50 dark:hover:bg-red-900/20 transition-all duration-200" title="Supprimer">
                    ...
                    <tr>
                        <td colspan="4" class="px-6 py-20 text-center">
                            <div class="flex flex-col items-center gap-5">
                                <span class="flex items-center justify-center w-20 h-20 rounded-2xl bg-slate-100 dark:bg-slate-700/80 text-slate-400 dark:text-slate-500 text-3xl">🎖️</span>
                                <div>
                                    <p class="font-bold text-slate-700 dark:text-slate-300 text-lg">Aucune fonction métier</p>
                                    <p class="text-sm text-slate-500 dark:text-slate-400 mt-1">Créez une fonction ou exécutez les seeders</p>
                                </div>
                                <a href="{{ route('parametres.fonctions.create') }}" class="inline-flex items-center gap-2 px-4 py-2 rounded-lg bg-[#00b464] text-white text-sm font-semibold hover:bg-[#00a055] shadow-sm hover:shadow transition-all no-underline">
    ...
    <div class="flex flex-wrap gap-4">
        <a href="{{ route('parametres.index') }}" class="inline-flex items-center gap-2 px-5 py-2.5 rounded-xl border-2 border-slate-300 dark:border-slate-600 text-slate-700 dark:text-slate-200 font-semibold hover:bg-slate-50 dark:hover:bg-slate-700 no-underline transition-colors">
