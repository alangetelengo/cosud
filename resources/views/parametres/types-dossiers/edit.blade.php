...
@section('page-title', 'Modifier le type de dossier')
...
<div class="flex flex-row gap-8 w-full items-stretch">
    ...
    <form id="form-type-dossier-edit" action="{{ route('parametres.types-dossiers.update', $type) }}" method="POST">
        @csrf
        @method('PUT')
        <div class="space-y-6">
            <div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="text" name="icone_defaut" value="{{ old('icone_defaut', $type->icone_defaut) }}" maxlength="50" class="w-full px-4 py-2 rounded-lg border border-slate-300 dark:border-slate-600 dark:bg-slate-700 dark:text-white">
                </div>
                <div>
                    <label class="block text-sm font-semibold text-slate-700 dark:text-slate-300 mb-2">Couleur (#hex)</label>
                    <input type="text" name="couleur_defaut" value="{{ old('couleur_defaut', $type->couleur_defaut) }}" pattern="#[A-Fa-f0-9]{6}" maxlength="7" class="w-full px-4 py-2 rounded-lg border border-slate-300 dark:border-slate-600 dark:bg-slate-700 dark:text-white @error('couleur_defaut') border-red-500 @enderror">
        ...
        <div class="mt-8 flex gap-4">
            <button type="submit" id="btn-type-dossier-edit-submit" class="px-6 py-2.5 rounded-lg bg-[#00b464] text-white font-semibold hover:bg-[#00a055] btn-submit-loading">Mettre à jour</button>
    ...
    <aside class="flex-1 min-w-0 sticky top-24 bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-700 p-6">
        ...
        <ul class="space-y-3 text-sm text-slate-600 dark:text-slate-400">
            <li><strong class="text-slate-800 dark:text-slate-200">Modification</strong> — Modifiez le code, le libellé, l'icône ou la couleur. Les dossiers existants conservent ce type ; les changements s'appliquent aux nouveaux dossiers.</li>
        </ul>
    </aside>
</div>
@push('styles')
...
#btn-type-dossier-edit-submit .btn-submit-spinner,
#btn-type-dossier-edit-submit .auth-spinner {
    ...
    animation: type-dossier-edit-spin 0.6s linear infinite !important;
    vertical-align: -0.2em !important;
    margin-right: 0.35rem !important;
    flex-shrink: 0 !important;
}
@keyframes type-dossier-edit-spin {
...
@push('scripts')
<script>
(function(){var f=document.getElementById('form-type-dossier-edit'),b=document.getElementById('btn-type-dossier-edit-submit');if(!f||!b)return;f.addEventListener('submit',function(){if(b.dataset.loading==='1')return;b.dataset.loading='1';b.innerHTML='<span class="btn-submit-spinner"></span> Mise à jour...';b.disabled=true;});})();
</script>
@endpush
@endsection
