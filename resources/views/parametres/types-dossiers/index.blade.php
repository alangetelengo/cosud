...
@section('page-title', 'Types de dossiers')
@section('page-title-info')
    <nav class="flex items-center gap-1.5 text-sm flex-wrap" aria-label="Fil d'Ariane">
        ...
        <span class="text-slate-700 dark:text-slate-200 font-semibold">Types de dossiers</span>
    </nav>
@endsection

@section('btn-create')
    <a href="{{ route('parametres.types-dossiers.create') }}" class="inline-flex items-center gap-2 px-5 py-2.5 rounded-lg bg-[#00b464] text-white font-semibold hover:bg-[#00a055] shadow-sm hover:shadow transition-all no-underline">
            ...
            <table class="w-full">
                <thead>
                    <tr class="bg-gradient-to-r from-slate-50 to-slate-100/80 dark:from-slate-700/80 dark:to-slate-800/80 border-b-2 border-slate-200 dark:border-slate-600">
                        ...
                        <th class="px-6 py-4 text-left text-xs font-bold text-slate-600 dark:text-slate-300 uppercase tracking-widest hidden sm:table-cell">Icône / Couleur</th>
                    ...
                    <tr class="group hover:bg-emerald-50/50 dark:hover:bg-slate-700/40 transition-colors duration-200">
                        ...
                        <td class="px-6 py-4 hidden sm:table-cell">
                            <span class="inline-flex items-center gap-2">
                                @if($type->icone_defaut)
                                <span class="text-lg">{{ $type->icone_defaut }}</span>
                                @endif
                                @if($type->couleur_defaut)
                                <span class="w-6 h-6 rounded-full border border-slate-200 dark:border-slate-600" style="background-color: {{ $type->couleur_defaut }}"></span>
                                <span class="text-xs text-slate-500 font-mono">{{ $type->couleur_defaut }}</span>
                                @else
                                —
                                @endif
                            </span>
                        ...
                        <td class="px-6 py-4">
                            <div class="flex items-center justify-end gap-1">
                                <a href="{{ route('parametres.types-dossiers.edit', $type) }}"
                                ...
                                <form action="{{ route('parametres.types-dossiers.destroy', $type) }}" method="POST" class="inline">
                                    @csrf
                                    @method('DELETE')
                                    <button type="button" onclick="flashAlert('Supprimer ce type de dossier ?', this.closest('form'), {icon:'🗑️', danger:true, confirmText:'Supprimer'})" class="p-2.5 rounded-xl text-slate-400 hover:text-red-600 hover:bg-red-50 dark:hover:bg-red-900/20 transition-all duration-200" title="Supprimer">
                    ...
                    <tr>
                        <td colspan="5" class="px-6 py-20 text-center">
                            <div class="flex flex-col items-center gap-5">
                                ...
                                <div>
                                    <p class="font-bold text-slate-700 dark:text-slate-300 text-lg">Aucun type de dossier</p>
                                    <p class="text-sm text-slate-500 dark:text-slate-400 mt-1">Créez un type ou exécutez les seeders</p>
                                </div>
                                <a href="{{ route('parametres.types-dossiers.create') }}" class="inline-flex items-center gap-2 px-4 py-2 rounded-lg bg-[#00b464] text-white text-sm font-semibold hover:bg-[#00a055] shadow-sm hover:shadow transition-all no-underline">
