...
<div class="w-full">
    ...
    <div id="doc-edit-grid" class="grid grid-cols-1 md:grid-cols-12 gap-6">
        {{-- BLOC GAUCHE : Formulaire --}}
        <div id="doc-edit-form" class="md:col-span-5 flex flex-col min-w-0 order-2 md:order-1">
            <div class="bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-700 overflow-hidden">
                ...
                <form id="form-document-edit" action="{{ route('documents.update', $document) }}" method="POST" class="p-6 space-y-6">
                    @csrf
                    @method('PUT')
                    <div class="space-y-6">
                        <div>
                            ...
                            <select name="dossier_id" class="w-full px-4 py-2.5 rounded-xl border border-slate-200 dark:border-slate-600 dark:bg-slate-700 dark:text-slate-200 focus:ring-2 focus:ring-emerald-500/50 focus:border-emerald-500 transition-colors @error('dossier_id') border-red-500 @enderror">
                                ...
                                <option value="{{ $d->id }}" {{ old('dossier_id', $document->dossier_id) == $d->id ? 'selected' : '' }}>{{ $d->chemin_complet }}</option>
                                @endforeach
                            </select>
        ...
        {{-- BLOC DROITE : Aide + Fichier actuel --}}
        <div id="doc-edit-preview" class="md:col-span-7 flex flex-col gap-6 min-w-0 order-1 md:order-2">
            {{-- Panneau Aide --}}
            <aside class="bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-700 p-6">
                ...
                <p class="text-sm text-slate-600 dark:text-slate-400">Vous pouvez modifier le dossier, le type, le titre, la référence, les mots-clés et la description. Pour remplacer le fichier, cliquez sur <strong>Nouvelle version</strong> dans le bloc « Fichier actuel » à droite.</p>
            </aside>
            <div class="bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-700 overflow-hidden">
                <div class="px-6 py-4 border-b border-slate-100 dark:border-slate-700 bg-slate-50/50 dark:bg-slate-700/30">
                    <h3 class="font-semibold text-slate-800 dark:text-slate-200 flex items-center gap-2">
...
@media (min-width: 768px) {
    #doc-edit-grid { display: grid !important; grid-template-columns: 5fr 7fr !important; gap: 1.5rem !important; }
    #doc-edit-form { order: 1; }
    #doc-edit-preview { order: 2; }
}
