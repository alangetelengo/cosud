...
@push('styles')
<style>
/* Disposition deux blocs côte à côte — garanti même si Tailwind échoue */
@media (min-width: 768px) {
    ...
    #doc-create-form { order: 1; }
    #doc-create-preview { order: 2; }
}
...
<div class="w-full" x-data="documentDepot()">
    <form id="form-document-create" action="{{ route('documents.store') }}" method="POST" enctype="multipart/form-data">
        @csrf
        {{-- Deux blocs côte à côte : FORMULAIRE (gauche) | APERÇU + DROP (droite) — dès 768px --}}
        <div id="doc-create-grid" class="grid grid-cols-1 md:grid-cols-12 gap-6">
            {{-- BLOC GAUCHE : Formulaire (métadonnées + boutons) --}}
            <div id="doc-create-form" class="md:col-span-5 flex flex-col min-w-0 order-2 md:order-1">
            ...
            {{-- BLOC DROITE : Aperçu + Zone drag & drop --}}
            <div id="doc-create-preview" class="md:col-span-7 flex flex-col gap-6 min-w-0 order-1 md:order-2">
