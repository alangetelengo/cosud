                    ...
                    <tr class="hover:bg-slate-50/80 dark:hover:bg-slate-700/30 transition-colors">
                        ...
                        <td class="px-6 py-4">
                            ...
                            <span class="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-lg text-xs font-medium
                                ...
                                @endif
                                @if(in_array($statutCode, ['en_attente']) && is_array($doc->workflow_validation_chain) && count($doc->workflow_validation_chain) > 1)
                                <span class="ml-1 opacity-80">({{ ($doc->workflow_etape_index ?? 0) + 1 }}/{{ count($doc->workflow_validation_chain) }})</span>
                                @endif
                            </span>
                        </td>
                        <td class="px-6 py-4">
                            ...
                            <div class="flex items-center justify-end gap-1">
                                ...
                                @php
                                    $wfCtx = $doc->type_document_id
                                        ? ($workflowContextByTypeId[$doc->type_document_id] ?? \App\Models\WorkflowEtape::contexteEnvoiPourType($doc->type_document_id, $doc->typeDocument?->libelle))
                                        : $workflowContextSansType;
                                @endphp
                                <button type="button"
                                    data-doc-id="{{ $doc->id }}"
                        ...
                        <div class="absolute left-0 right-0 top-full z-[10002] mt-1.5 flex max-h-80 min-w-0 flex-col overflow-hidden rounded-xl border border-slate-200 bg-white shadow-2xl ring-1 ring-slate-900/5 dark:border-slate-600 dark:bg-slate-800 dark:ring-white/10">
                            ...
                            <div class="max-h-60 flex-1 overflow-y-auto p-1.5">
                                ...
                                <template x-for="opt in filteredOptions()" :key="'env-'+opt.value">
                                    <button type="button" @click.stop="select(opt)" class="w-full truncate rounded-lg px-3 py-2 text-left text-sm text-slate-800 hover:bg-emerald-50 dark:text-slate-200 dark:hover:bg-emerald-900/25" x-text="opt.label"></button>
                                </template>
...
function remplirEnvoiWorkflowDepuisBouton(btn) {
    ...
    var typeLib = btn.getAttribute('data-workflow-type-libelle') || '';
    ...
    if (infoEl) {
        var parts = [];
        if (source === 'type' && typeLib) {
            parts.push('Circuit défini pour le type « ' + typeLib + ' » (prioritaire sur le workflow global).');
        } else if (source === 'global') {
            parts.push('Aucun circuit spécifique pour ce type : le workflow global s’applique.');
        }
        if (source && etapes.length) {
            parts.push('Le destinataire choisi sera le premier valideur de la première étape ci-dessous.');
        } else if (!source) {
