...
- **Comptes de test** (ex. via seeder) :
  ...
  - **Démo DDSAIT et DG** :
    - `ingenieur.ddsait@acsi.cg` / `password` (Ingénieur DDSAIT – créateur)
    - `dir.ddsait@acsi.cg` / `password` (Directeur DDSAIT – validateur hiérarchique)
  - **Commun** :
...
2. Se connecter avec `agent.daf@acsi.cg` ou `ingenieur.ddsait@acsi.cg` (démo DDSAIT) / `password`.
...
### Points à mettre en avant
> « Le plan de classement reflète l’organisation. Chaque dossier est rattaché à une structure (ex. DAF, DDSAIT). Le propriétaire est le responsable de la structure, donc un acteur métier, pas l’admin. »
...
## Variante : Démo DDSAIT et DG

Pour une démo axée sur la direction DDSAIT et le DG :

| Compte | Rôle | Usage |
|--------|------|-------|
| `ingenieur.ddsait@acsi.cg` | Ingénieur DDSAIT | Créateur – dépose dans Projets / Cahier des charges |
| `dir.ddsait@acsi.cg` | Directeur DDSAIT | Validateur hiérarchique |
| `dg@acsi.cg` | DG | Vue globale |

**Arborescence à parcourir** : Projets → Projet A → Cahier des charges *(ou* Ingénierie SI → Documentation *)*.

**Chaîne de validation** : Ingénieur DDSAIT → Directeur DDSAIT → DG.
