...
use App\Notifications\DossierPartageRemovedNotification;
...
class DossierController extends Controller
{
    public function index(Request $request)
    {
        ...
        // Aligné sur DossierPolicy::view : un dossier confidentiel reste visible dans l’arbre pour son créateur
        // ou le propriétaire, même sans la permission globale dossiers.view-confidentiel (sinon la racine
        // « disparaît » après création alors que la redirection show fonctionne).
        $dossiers = $dossiers->filter(fn ($d) => $d->visiblePar($user) && $this->utilisateurPeutVoirNoeudConfidentiel($user, $d))
        ...
        if ($filtre === 'mes') {
            ...
            $dossiersPartages = Dossier::query()
                ->whereHas('partages', function ($pq) use ($user) {
        ...
        $dossiersPartages = collect();

        return view('dossiers.index', compact('dossiers', 'dossiersFavoris', 'dossiersRecents', 'dossiersPartages', 'favoriIds', 'filtre', 'q'));
    }
    ...
    public function store(Request $request, MesDossiersRacineService $mesDossiersRacine)
    {
        ...
        if ($creerRacine) {
            ...
            if (! $parent) {
                ...
                if ($user->structure_id) {
                    $defaultParent = Dossier::query()
                        ...
                        ->where('structure_id', (int) $user->structure_id)
                        ->orderBy('ordre')
                        ->first();
                }
            ...
            if ($sousArbrePersonnel) {
                ...
                if (! $user->can('dossiers.create-structure')) {
                    return back()->withInput()->withErrors([
                        'parent_id' => 'Vous pouvez créer sans permission structure uniquement dans votre espace « Mes dossiers ».',
                    ]);
                }
                $parentStructureId = $parent->structure_id ?? $parent->structure_id_depot;
        ...
        if ($isProjectType && $user->can('share', $dossier)) {
            return redirect()
                ->route('dossiers.partages', $dossier)
                ->with('success', 'Dossier projet créé. Partagez-le maintenant au chef de projet et à son équipe (droits écriture).');
        }
    ...
    private function listeParentsPourStructure(User $user, ?Dossier $racine): Collection
    {
        ...
        return Dossier::where('actif', true)
            ...
            ->map(function (Dossier $d) use ($user, $racinePersonnelleId) {
                $label = $d->chemin_complet;
                $estRacinePerso = ((int) $d->id === $racinePersonnelleId)
                    || ((int) ($d->racine_utilisateur_id ?? 0) === (int) $user->id && $d->parent_id === null);
                if ($estRacinePerso) {
            ...
            ->sortBy(function ($opt) use ($racinePersonnelleId) {
                if ($racinePersonnelleId > 0 && (int) $opt->value === $racinePersonnelleId) {
                    return '0';
                }
    ...
    private function listeParentsPourEdition(User $user, ?Dossier $racine, Dossier $dossier): Collection
    {
        ...
        if ($isOwner) {
            return Dossier::query()
                ->where('actif', true)
                ->get()
                ->filter(fn (Dossier $d) => $d->visiblePar($user))
    ...
    {
        if ($racine !== null && Dossier::estSousRacineMesDossiers($dossier, $racine)) {
            return true;
        }

        if ((int) ($dossier->racine_utilisateur_id ?? 0) === (int) $user->id) {
            return true;
        }

        return in_array((int) $dossier->id, Dossier::idsPourArbrePersonnel((int) $user->id), true);
    }

    private function parentAutorisePourDeplacement(User $user, Dossier $parent, ?Dossier $racine, Dossier $dossier): bool
    {
        $isOwner = (int) $dossier->proprietaire_id === (int) $user->id;
        if ($isOwner && $parent->visiblePar($user)) {
            return true;
        }

        return $this->parentAutorisePourCreation($user, $parent, $racine);
    }
    ...
    {
        $this->authorize('share', $dossier);
        // Compat : l'ancienne version utilisait `user_id` (1 seul utilisateur),
        // la nouvelle utilise `user_ids[]` (multi-sélection).
        $userIdsInput = $request->input('user_ids');
        if ($userIdsInput === null) {
            $userIdsInput = $request->filled('user_id') ? [$request->input('user_id')] : [];
        }
        $request->merge(['user_ids' => $userIdsInput]);
        ...
        try {
            $request->validate([
                ...
                'appliquer_sous_dossiers' => ['sometimes', 'boolean'],
                'propager_aux_sous_dossiers' => ['sometimes', 'boolean'],
        ...
        $selectedUserIds = array_values(array_unique(array_map('intval', $request->input('user_ids', []))));
        $appliquerSousDossiers = $request->boolean('appliquer_sous_dossiers');
        $propagerAuxSousDossiers = $request->boolean('propager_aux_sous_dossiers');

        $targetDossierIds = [(int) $dossier->id];
        if ($appliquerSousDossiers) {
            $descendantsIds = $this->idsDescendantsDossier((int) $dossier->id);
            if (! empty($descendantsIds)) {
                $shareableDescendants = Dossier::query()
                    ->whereIn('id', $descendantsIds)
                    ->get(['id'])
                    ->filter(fn (Dossier $d) => auth()->user()->can('share', $d))
                    ->pluck('id')
                    ->map(fn ($id) => (int) $id)
                    ->all();
                $targetDossierIds = array_values(array_unique(array_merge($targetDossierIds, $shareableDescendants)));
            }
        }

        $existingPairs = DossierPartage::query()
            ->whereIn('dossier_id', $targetDossierIds)
            ->whereIn('user_id', $selectedUserIds)
            ->get(['dossier_id', 'user_id'])
            ->map(fn ($p) => ((int) $p->dossier_id).':'.((int) $p->user_id))
            ->all();
        ...
        $dateExpiration = $request->filled('date_expiration') ? $request->date('date_expiration') : null;
        $toCreatePairs = [];
        foreach ($targetDossierIds as $did) {
            foreach ($selectedUserIds as $uid) {
                $key = ((int) $did).':'.((int) $uid);
                if (! in_array($key, $existingPairs, true)) {
                    $toCreatePairs[] = [(int) $did, (int) $uid];
                }
            }
        }
        ...
        if ($toCreatePairs === []) {
            Log::channel('eged')->notice('dossier.partage.store refusé : tous déjà partagés', [
                ...
                'user_id' => auth()->id(),
                'selection' => $selectedUserIds,
                'dossiers_cibles' => $targetDossierIds,
            ]);

            return back()->with('error', 'Tous les utilisateurs sélectionnés ont déjà un partage sur la portée choisie.');
        }

        try {
            $createdByUser = [];
            foreach ($toCreatePairs as [$did, $uid]) {
                DossierPartage::create([
                    ...
                    'propager_aux_sous_dossiers' => $propagerAuxSousDossiers,
        ...
        $partagePar = auth()->user();
        if (! empty($createdByUser)) {
            $destinataires = User::query()
                ->where('actif', true)
                ->whereIn('id', array_keys($createdByUser))
                ->get(['id', 'name', 'email']);
            foreach ($destinataires as $destinataire) {
                try {
                    $destinataire->notify(new DossierPartageNotification(
                        $dossier->fresh(),
                        $partagePar,
                        (int) ($createdByUser[(int) $destinataire->id] ?? 1),
                        [
                            'lecture' => $lecture,
                            'ecriture' => $ecriture,
        ...
        Log::channel('eged')->info('dossier.partage.store partages créés', [
            ...
            'dossiers_cibles' => $targetDossierIds,
            'destinataires_user_ids' => $selectedUserIds,
            'appliquer_sous_dossiers' => $appliquerSousDossiers,
            'propager_aux_sous_dossiers' => $propagerAuxSousDossiers,
            'droits' => ['lecture' => $lecture, 'ecriture' => $ecriture, 'suppression' => $suppression],
            'date_expiration' => $dateExpiration?->format('Y-m-d'),
        ]);

        return redirect()->route('dossiers.partages', $dossier)
            ->with('success', 'Partage(s) ajouté(s) : '.count($toCreatePairs).' attribution(s) créées.');
    }
    ...
    public function updatePartage(Request $request, Dossier $dossier, DossierPartage $partage)
    {
        ...
        try {
            $partage->update([
                ...
                'droits_ecriture' => $ecriture,
                'droits_suppression' => $suppression,
                'propager_aux_sous_dossiers' => $request->boolean('propager_aux_sous_dossiers'),
        ...
        $destinataire = User::query()
            ->where('actif', true)
            ->whereKey((int) $partage->user_id)
            ->first(['id', 'name', 'email']);
        if ($destinataire) {
            try {
                $destinataire->notify(new DossierPartageUpdatedNotification(
                    $dossier->fresh(),
                    auth()->user(),
                    [
                        'lecture' => $lecture,
        ...
        Log::channel('eged')->info('dossier.partage.update partage modifié', [
            ...
            'destinataire_user_id' => (int) $partage->user_id,
            'droits' => ['lecture' => $lecture, 'ecriture' => $ecriture, 'suppression' => $suppression],
            'propager_aux_sous_dossiers' => (bool) $partage->propager_aux_sous_dossiers,
            'date_expiration' => $partage->date_expiration?->format('Y-m-d'),
        ]);
    ...
    public function destroyPartage(Dossier $dossier, int $partage)
    {
        ...
        if ($destinataire) {
            try {
                $destinataire->notify(new DossierPartageRemovedNotification(
                    $dossier->fresh(),
                    auth()->user()
                ));
            } catch (\Throwable $e) {
    ...
    /** Hérite les partages d'ancêtres marqués "propager aux sous-dossiers" lors de la création d'un nouveau dossier. */
    private function appliquerPartagesHereditesDepuisAncetres(Dossier $dossier, int $acteurId): void
    {
        if (! $dossier->parent_id) {
            return;
        }

        $ancetreIds = $this->idsAncetresDuDossier((int) $dossier->id);
        if ($ancetreIds === []) {
            return;
        }

        $partagesAncetres = DossierPartage::query()
            ->whereIn('dossier_id', $ancetreIds)
            ->where('propager_aux_sous_dossiers', true)
            ->get();

        if ($partagesAncetres->isEmpty()) {
            return;
        }

        $existingUserIds = DossierPartage::query()
            ->where('dossier_id', $dossier->id)
            ->pluck('user_id')
            ->map(fn ($id) => (int) $id)
            ->all();

        foreach ($partagesAncetres as $partage) {
            $uid = (int) $partage->user_id;
            ...
            DossierPartage::create([
                'dossier_id' => (int) $dossier->id,
                'user_id' => $uid,
                'partage_par_id' => $acteurId,
                'droits_lecture' => (bool) $partage->droits_lecture,
                'droits_ecriture' => (bool) $partage->droits_ecriture,
                'droits_suppression' => (bool) $partage->droits_suppression,
                'propager_aux_sous_dossiers' => true,
                'date_expiration' => $partage->date_expiration,
            ]);
            $existingUserIds[] = $uid;
        }
    }
    ...
    /** @return list<int> */
    private function idsAncetresDuDossier(int $dossierId): array
    {
        $ids = [];
        $current = Dossier::query()->select(['id', 'parent_id'])->find($dossierId);
        while ($current && $current->parent_id) {
            $pid = (int) $current->parent_id;
            $ids[] = $pid;
            $current = Dossier::query()->select(['id', 'parent_id'])->find($pid);
        }
    ...
    public function edit(Dossier $dossier, MesDossiersRacineService $mesDossiersRacine)
    {
        ...
        if ($peutChangerParent) {
            if ($dossier->estDansArbrePersonnelMesDossiers() && $racine) {
                ...
                $isOwner = (int) $dossier->proprietaire_id === (int) $user->id;
                $peutChoisirRacineOrg = $isOwner || $user->aAccesTotal()
                    || $user->can('dossiers.create-racine-structure');
    ...
    public function update(Request $request, Dossier $dossier, MesDossiersRacineService $mesDossiersRacine)
    {
        ...
        if ($dossier->racine_utilisateur_id === null && $wantedParent !== null) {
            ...
            if (! $this->parentAutorisePourDeplacement($user, $parentModel, $racine, $dossier)) {
        ...
        if ($wantedParent === null && ! $dossier->estDansArbrePersonnelMesDossiers()) {
            ...
            $isOwner = (int) $dossier->proprietaire_id === (int) $user->id;
            if ($dossier->parent_id !== null && ! $peutRacine && ! $isOwner) {
    ...
    private function filtrerEnfantsVisibles(Dossier $dossier, $user): void
    {
        ...
        $dossier->setRelation('children', $dossier->children->filter(fn (Dossier $c) => $c->visiblePar($user) && $this->utilisateurPeutVoirNoeudConfidentiel($user, $c))
            ->each(fn ($c) => $this->filtrerEnfantsVisibles($c, $user)));
    }

    /** Même règle que {@see DossierPolicy::view} pour les dossiers confidentiels dans l’arbre du plan. */
    private function utilisateurPeutVoirNoeudConfidentiel(User $user, Dossier $d): bool
    {
        if (! $d->confidentiel) {
            return true;
        }
        ...
        return (int) $d->createur_id === (int) $user->id
            || (int) $d->proprietaire_id === (int) $user->id;
    }
